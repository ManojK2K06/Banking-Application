...
public class PartyPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        JButton refreshBtn = Theme.ghostButton("Reload");
        ...
        searchBtn.addActionListener(
                e -> loadParties(searchField.getText().trim(), (String) typeFilter.getSelectedItem()));
    ...
    private void loadParties(String query, String typeF) {
        new SwingWorker<List<Party>, Void>() {
            @Override
            protected List<Party> doInBackground() throws Exception {
            ...
            protected void done() {
                try {
                    ...
                    Theme.showError(PartyPanel.this, ex.getMessage());
                }
            }
        }.execute();
    }
    ...
    private Party getSelected() {
        ...
        if (row < 0 || currentList == null) {
            Theme.showError(this, "Select a party first");
            return null;
        }
    ...
    private void showPartyDialog(Party existing, String partyType) {
        ...
        for (String s : new String[] { "RETAIL", "SME", "CORPORATE", "HNI" })
            segmentBox.addItem(s);
        ...
        for (String g : new String[] { "Male", "Female", "Other", "Not Disclosed" })
            genderBox.addItem(g);
        ...
        for (String b : new String[] { "Private Limited", "Public Limited", "LLP", "OPC", "Sole Proprietorship",
                "Partnership", "Trust", "NGO", "Government" })
            bizType.addItem(b);

        if (isEdit) {
            email.setText(existing.getEmail());
            phone.setText(existing.getPhone());
            if (existing.getAltPhone() != null)
                altPhone.setText(existing.getAltPhone());
            ...
            if (existing.getPincode() != null)
                pincode.setText(existing.getPincode());
            if (existing.getPanNumber() != null)
                pan.setText(existing.getPanNumber());
            if (existing.getSegment() != null)
                segmentBox.setSelectedItem(existing.getSegment());
            if (!isCorp) {
                ...
                if (existing.getAadharNumber() != null)
                    aadhar.setText(existing.getAadharNumber());
            } else {
                if (existing.getCompanyName() != null)
                    companyName.setText(existing.getCompanyName());
                if (existing.getRegistrationNumber() != null)
                    regNo.setText(existing.getRegistrationNumber());
                if (existing.getIncorporationDate() != null)
                    incDate.setText(existing.getIncorporationDate());
                if (existing.getGstNumber() != null)
                    gst.setText(existing.getGstNumber());
                if (existing.getCinNumber() != null)
                    cin.setText(existing.getCinNumber());
                if (existing.getContactPersonName() != null)
                    contactPerson.setText(existing.getContactPersonName());
                if (existing.getContactPersonDesig() != null)
                    contactDesig.setText(existing.getContactPersonDesig());
            }
        }
        ...
        gc.gridy = row[0];
        gc.gridwidth = 2;
        p.add(Theme.heading((isEdit ? "Edit " : "Register ") + partyType), gc);
        row[0]++;
        gc.gridwidth = 1;
        ...
        saveBtn.addActionListener(e -> {
            try {
                ...
                if (isEdit)
                    PartyService.getInstance().updateParty(party);
                else
                    PartyService.getInstance().createParty(party);
                dlg.dispose();
                loadParties("", "ALL");
    ...
    private void viewAccounts() {
        ...
        try {
            ...
            if (accounts.isEmpty()) {
                Theme.showError(this, "No accounts found for " + p.getDisplayName());
                return;
            }
