...
class FixedDepositPanelImpl extends JPanel {
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            ResultSet rs = stmt.executeQuery(
                    "SELECT fd.*, COALESCE(c.first_name||' '||c.last_name, c.company_name) AS cname FROM fixed_deposits fd "
                            + "LEFT JOIN parties c ON fd.party_id=c.id ORDER BY fd.created_at DESC");
            while (rs.next()) {
    ...
    private void showCreateFD() {
        ...
        for (String t : new String[] { "6", "12", "24", "36", "60", "84", "120" })
            tenor.addItem(t);
        JTextField customTenure = Theme.styledField();
        customTenure.setToolTipText("Or enter custom months");
        JLabel maturityLabel = new JLabel("Maturity: —");
        maturityLabel.setForeground(Theme.ACCENT_GOLD);
        JLabel custInfo = Theme.label("—");
        JLabel acctInfo = Theme.label("—");
        final int[] cId = { -1 }, aId = { -1 };

        int[] row = { 0 };
        addRow(p, gc, row, "Party ID / Name / PAN", custSearch);
        ...
        calcBtn.addActionListener(e -> {
            try {
                ...
                maturityLabel.setText("Invalid input");
            }
        });
        createBtn.addActionListener(e -> {
            if (cId[0] < 0 || aId[0] < 0) {
                Theme.showError(dlg, "Select party & account");
                return;
            }
            if (!BankUtil.isValidAmount(amount.getText())) {
                Theme.showError(dlg, "Invalid amount");
                return;
            }
            try {
                double pr = Double.parseDouble(amount.getText().trim());
                ...
                if (a.getBalance() < pr) {
                    Theme.showError(dlg, "Insufficient balance: " + BankUtil.formatCurrency(a.getBalance()));
                    return;
                }

                try (Connection conn = DatabaseManager.getInstance().getConnection();
                        PreparedStatement ps = conn.prepareStatement(
                                "INSERT INTO fixed_deposits (fd_number,party_id,account_id,principal_amount," +
                ...
                dlg.dispose();
                loadFDs();
                Theme.showSuccess(this, "FD Created!\nFD No: " + fdNo + "\nMaturity: " + BankUtil.formatCurrency(mat)
                        + " on " + matDate);
            } catch (Exception ex) {
                Theme.showError(dlg, ex.getMessage());
            }
        ...
        dlg.add(new JScrollPane(p) {
            {
                setBackground(Theme.BG_CARD);
                getViewport().setBackground(Theme.BG_CARD);
                setBorder(null);
            }
        });
        dlg.setVisible(true);
    ...
    private void breakFD() {
        ...
        if (row < 0) {
            Theme.showError(this, "Select an FD");
            return;
        }
        String fdNo = tableModel.getValueAt(row, 0).toString();
        if (!Theme.showConfirm(this, "Break FD " + fdNo + "? A 1% penalty applies on premature withdrawal."))
            return;
        try (Connection conn = DatabaseManager.getInstance().getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
...
class CardsPanelImpl extends JPanel {
    private DefaultTableModel tableModel;
    private static final String[] COLS = { "Card Number", "Holder", "Type", "Network", "Account", "Expiry", "Status",
            "Limit" };
    ...
    private void issueCard(JTable table) {
        ...
        for (String t : new String[] { "DEBIT", "CREDIT", "PREPAID" })
            type.addItem(t);
        JComboBox<String> network = Theme.styledCombo();
        for (String n : new String[] { "VISA", "MASTERCARD", "RUPAY" })
            network.addItem(n);
        JTextField creditLimit = Theme.styledField();
        ...
        accountNo.addActionListener(e -> {
            try {
                ...
                if (a != null) {
                    aId[0] = a.getId();
                    holderName.setText(a.getPartyName() != null ? a.getPartyName() : "");
                    acctInfo.setText("✓ " + (a.getPartyName() != null ? a.getPartyName() : a.getAccountNumber()));
                    acctInfo.setForeground(Theme.ACCENT_GREEN);
                } else {
                    acctInfo.setText("❌ Not found");
                    aId[0] = -1;
        ...
        issueBtn.addActionListener(e -> {
            if (aId[0] < 0) {
                Theme.showError(dlg, "Verify account first");
                return;
            }
            if (holderName.getText().trim().isEmpty()) {
                Theme.showError(dlg, "Enter card holder name");
                return;
            }
            try {
                ...
                try (Connection conn = DatabaseManager.getInstance().getConnection();
                    ...
                    ps.setString(4, holderName.getText().trim());
                    ps.setString(5, expiry);
                    ps.setString(6, cvvHash);
                    ps.setDouble(7, limit);
                    ps.setString(8, (String) network.getSelectedItem());
                    ps.executeUpdate();
                }
                dlg.dispose();
                loadCards();
                ...
                AuditService.getInstance().log("ISSUE_CARD", "CARD", cardNo, null, "ACTIVE",
                        "Card issued to " + holderName.getText());
            } catch (Exception ex) {
                Theme.showError(dlg, ex.getMessage());
            }
        });
    ...
    private void updateCardStatus(JTable table, String status) {
        int row = table.getSelectedRow();
        if (row < 0) {
            Theme.showError(this, "Select a card");
            return;
        }
        String masked = tableModel.getValueAt(row, 0).toString();
        if (!Theme.showConfirm(this, "Set card " + masked + " to " + status + "?"))
            return;
        try (Connection conn = DatabaseManager.getInstance().getConnection();
                PreparedStatement ps = conn.prepareStatement(
...
class ForexPanelImpl extends JPanel {
    ...
    private JPanel buildConverterCard() {
        ...
        amtField.setText("1000");
        JComboBox<String> from = Theme.styledCombo();
        JComboBox<String> to = Theme.styledCombo();
        JLabel result = new JLabel("—");
        result.setFont(Theme.FONT_AMOUNT);
        result.setForeground(Theme.ACCENT_GOLD);
        ...
        try {
            ...
            from.setSelectedItem("USD");
            to.setSelectedItem("INR");
        ...
        convertBtn.addActionListener(e -> {
            try {
                ...
                result.setForeground(Theme.ACCENT_RED);
            }
        });
        return card;
    }
    ...
    private JPanel buildSwiftCard() {
        ...
        sendBtn.addActionListener(e -> {
            try {
                ...
                if (a == null) {
                    Theme.showError(card, "Account not found");
                    return;
                }
                if (!BankUtil.isValidAmount(amtField.getText())) {
                    Theme.showError(card, "Invalid amount");
                    return;
                }
                double amt = Double.parseDouble(amtField.getText().trim());
                if (!Theme.showConfirm(card, "Send " + amt + " " + curCombo.getSelectedItem() + " internationally?"))
                    return;
                ...
                Theme.showError(card, ex.getCause() != null ? ex.getCause().getMessage() : ex.getMessage());
            }
        });
        return card;
    }
...
class AuditLogPanelImpl extends JPanel {
    ...
    private static final String[] COLS = { "#", "User", "Action", "Entity", "Entity ID", "Status", "Timestamp",
            "Details" };
...
class UserManagementPanelImpl extends JPanel {
    ...
    private static final String[] COLS = { "Emp ID", "Username", "Name", "Email", "Level", "Role", "Dept", "Status",
            "Last Login" };
    ...
    private void buildUI() {
        ...
        bottom.add(changeLevel);
        bottom.add(resetPwd);
        bottom.add(deactivate);
    ...
    private User getSelected(JTable table) {
        int row = table.getSelectedRow();
        if (row < 0 || users == null) {
            Theme.showError(this, "Select a user");
            return null;
        }
    ...
    private void resetPassword(JTable table) {
        ...
        int res = JOptionPane.showConfirmDialog(this, pwdField, "New password for " + u.getUsername(),
...
class SettingsPanelImpl extends JPanel {
    ...
    private void buildUI() {
        ...
        if (SessionManager.getInstance().hasPermission(7)) {
            gc.gridy = row[0];
            gc.insets = new Insets(8, 8, 8, 8);
            card.add(Theme.heading("Update Currency Rate"), gc);
            row[0]++;

            JComboBox<String> codeCombo = Theme.styledCombo();
            try {
                for (String code : TransactionService.getInstance().getAllRates().keySet())
            ...
            gc.gridy = row[0];
            gc.weightx = 0.35;
            card.add(Theme.label("Currency Code"), gc);
            gc.gridx = 1;
            gc.weightx = 0.65;
            card.add(codeCombo, gc);
            row[0]++;
            gc.gridx = 0;
            gc.gridy = row[0];
            card.add(Theme.label("Rate to INR"), gc);
            gc.gridx = 1;
            card.add(rateField, gc);
            row[0]++;
            gc.gridx = 0;
            gc.gridy = row[0];
            gc.gridwidth = 2;
            ...
            updateRateBtn.addActionListener(e -> {
                try {
                    ...
                    Theme.showError(card, ex.getMessage());
                }
            });
        }
        ...
        changePwdBtn.addActionListener(e -> {
            ...
            if (!n.equals(cf)) {
                Theme.showError(card, "Passwords do not match");
                return;
            }
            try {
...
// ═══════════════════════════════════════════════════════════════════════════
public class FixedDepositPanel extends FixedDepositPanelImpl {
    public FixedDepositPanel() {
        super();
    }
}

class CardPanelInner extends CardsPanelImpl {
    CardPanelInner() {
        super();
    }
}

class ForexPanelInner extends ForexPanelImpl {
    ForexPanelInner() {
        super();
    }
}

class AuditLogPanelInner extends AuditLogPanelImpl {
    AuditLogPanelInner() {
        super();
    }
}

class UserManagementPanelInner extends UserManagementPanelImpl {
    UserManagementPanelInner() {
        super();
    }
}

class SettingsPanelInner extends SettingsPanelImpl {
    SettingsPanelInner() {
        super();
    }
}
