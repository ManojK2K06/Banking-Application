...
public class LoanPanel extends JPanel {
    ...
    private static final String[] COLS = {
            "Loan ID", "Party", "Type", "Principal", "Outstanding", "EMI", "Rate%", "Tenure", "Status", "Next EMI"
    };
    ...
    private void buildUI() {
        ...
        for (String s : new String[] { "ALL", "PENDING", "APPROVED", "ACTIVE", "CLOSED", "REJECTED" })
            filterCombo.addItem(s);
        JButton applyBtn = Theme.successButton("+ Apply Loan");
        JButton refreshBtn = Theme.ghostButton("⟳");
        acts.add(new JLabel("Filter: ") {
            {
                setForeground(Theme.TEXT_SECONDARY);
            }
        });
        acts.add(filterCombo);
        acts.add(applyBtn);
        acts.add(refreshBtn);
        ...
        bottomBar.add(disburseBtn);
        bottomBar.add(repayBtn);
        bottomBar.add(calcBtn);
                ...
                try {
                    ...
                    for (Loan l : currentList) {
                        tableModel.addRow(new Object[] {
                                l.getLoanId(), l.getPartyName() != null ? l.getPartyName() : l.getCustomerName(),
                                l.getLoanType(),
                    ...
                    Theme.showError(LoanPanel.this, ex.getMessage());
                }
            }
        }.execute();
    }
    ...
    private Loan getSelected() {
        ...
        if (row < 0 || currentList == null) {
            Theme.showError(this, "Select a loan first");
            return null;
        }
    ...
    private void showApplyDialog() {
        ...
        for (String t : new String[] { "PERSONAL", "HOME", "CAR", "EDUCATION", "BUSINESS", "GOLD", "MORTGAGE" })
            typeBox.addItem(t);
        ...
        gc.gridy = row;
        gc.gridwidth = 2;
        p.add(customerFound, gc);
        row++;
        gc.gridy = row;
        p.add(accountInfo, gc);
        row++;
        gc.gridy = row;
        p.add(emiPreview, gc);
        row++;
        ...
        searchBtn.addActionListener(e -> {
            try {
                ...
                if (res.isEmpty()) {
                    customerFound.setText("❌ Not found");
                    return;
                }
                Party c;
                if (res.size() == 1) {
                ...
                if (c == null)
                    return;
                cId[0] = c.getId();
                customerFound.setText(
                        "✓ " + c.getDisplayName() + " [" + c.getPartyId() + "] — Credit: " + c.getCreditScore());
        ...
        accountField.addActionListener(e -> {
            try {
                ...
                if (a == null) {
                    accountInfo.setText("❌ Account not found");
                    aId[0] = -1;
                    return;
                }
                aId[0] = a.getId();
        ...
        submitBtn.addActionListener(e -> {
            if (cId[0] < 0) {
                Theme.showError(dlg, "Search and select a party first");
                return;
            }
            if (aId[0] < 0) {
                Theme.showError(dlg, "Enter valid account number (press Enter to verify)");
                return;
            }
            if (!BankUtil.isValidAmount(amountField.getText())) {
                Theme.showError(dlg, "Invalid loan amount");
                return;
            }
            try {
    ...
    private void updateEmiPreview(JTextField amt, JTextField tenure, JTextField rate, JLabel label) {
        try {
            ...
            label.setText(
                    "EMI: " + BankUtil.formatCurrency(emi) + "/month  |  Total: " + BankUtil.formatCurrency(emi * t));
    ...
    private void approveLoan() {
        ...
        if (!"PENDING".equals(l.getStatus())) {
            Theme.showError(this, "Only PENDING loans can be approved");
            return;
        }
    ...
    private void rejectLoan() {
        ...
        if (l == null)
            return;
        if (!"PENDING".equals(l.getStatus())) {
            Theme.showError(this, "Only PENDING loans can be rejected");
            return;
        }
    ...
    private void disburseLoan() {
        ...
        if (!"APPROVED".equals(l.getStatus())) {
            Theme.showError(this, "Only APPROVED loans can be disbursed");
            return;
        }
        if (Theme.showConfirm(this, "Disburse " + BankUtil.formatCurrency(l.getPrincipalAmount())
            ...
            try {
                ...
                loadLoans((String) filterCombo.getSelectedItem());
                Theme.showSuccess(this, "Loan disbursed! Amount credited to party account.");
            } catch (Exception ex) {
                Theme.showError(this, ex.getMessage());
            }
        }
    ...
    private void repayLoan() {
        ...
        if (!"ACTIVE".equals(l.getStatus())) {
            Theme.showError(this, "Only ACTIVE loans can be repaid");
            return;
        }
        ...
        if (!BankUtil.isValidAmount(amtStr)) {
            Theme.showError(this, "Invalid amount");
            return;
        }
    ...
    private void showEmiCalculator() {
        ...
        JTextField principal = Theme.styledField();
        principal.setText("500000");
        JTextField rate = Theme.styledField();
        rate.setText("12.00");
        JTextField tenure = Theme.styledField();
        tenure.setText("60");
        JLabel result = new JLabel();
        result.setFont(Theme.FONT_HEADING);
